...
- `remix/assets` — browser asset server. Use for `createAssetServer` when serving compiled scripts and styles, getting public hrefs/import maps, and emitting preloads. Configure a `basePath` and `allowFiles` (required); `mounts` maps public path segments to directories (defaults to `{ app: 'app', npm: 'node_modules' }`). Shared compiler options such as `target`, `sourceMaps`, `sourceMapSourcePaths`, and `minify` live at the top level
